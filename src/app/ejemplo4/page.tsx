...
export default function ArcadeMenu() {
    ...
    return (
        <div className="min-h-screen bg-[#000033] text-white font-mono overflow-hidden relative selection:bg-fuchsia-500 selection:text-white">
            ...
            {/* CRT Scanline Overlay - reduced opacity for performance */}
            <div className="fixed inset-0 pointer-events-none bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0IiBoZWlnaHQ9IjQiPjxyZWN0IHdpZHRoPSI0IiBoZWlnaHQ9IjIiIGZpbGw9InJnYmEoMCwwLDAsMC4yKSIvPjwvc3ZnPg==')] z-50 opacity-30 mix-blend-overlay will-change-transform"></div>
            ...
            <AnimatePresence>
                {orderPlaced && (
                    ...
                    >
                        ...
                        >
                            <div className="text-6xl md:text-8xl font-black uppercase [text-shadow:3px_3px_0_#f0f,6px_6px_0_#0ff] mb-4">
                                ORDER PLACED!
                            </div>
                            ...
                            <div className="flex justify-center gap-2 mt-6">
                                {[...Array(5)].map((_, i) => (
                                    <motion.div
                                        ...
                                        className="w-3 h-3 bg-yellow-400 shadow-[0_0_10px_#ff0]"
                                    />
                                ))}
                            </div>
                        </motion.div>
            ...
            <main className="relative z-20 max-w-5xl mx-auto px-4 pt-28 pb-32 min-h-screen flex flex-col">
                ...
                <div className="flex flex-wrap justify-center gap-3 mb-12">
                    {["MAIN", "DRINK", "SWEET"].map((cat) => (
                        <button
                            ...
                            className={`px-6 py-2 border-2 text-sm font-bold tracking-widest uppercase transition-all duration-300
                ${activeCategory === cat
                    ...
                    {menuItems.filter(i => i.category === activeCategory).map((item) => {
                        ...
                        return (
                            <motion.div
                                ...
                                layout
                                whileTap={{ scale: 0.97 }}
                                className={`relative bg-black/60 backdrop-blur-sm border-2 overflow-hidden group cursor-pointer transition-all duration-200
                  ${isSelected
                            ...
                            >
                                <div className="p-4 md:p-6 flex gap-4 h-full relative z-20">
                                    <div className={`w-20 h-20 md:w-24 md:h-24 shrink-0 border-2 bg-[#000] flex items-center justify-center overflow-hidden
                    ${isSelected ? 'border-yellow-400 shadow-[0_0_10px_#ff0]' : 'border-cyan-500 group-hover:border-fuchsia-400'}`}>
                                    ...
                                    <div className="flex flex-col justify-between w-full h-full">
                                        <div>
                                            <div className="flex justify-between items-start mb-2">
                                                <h3 className={`font-bold text-lg leading-tight uppercase ${isSelected ? 'text-yellow-400' : 'text-white group-hover:text-fuchsia-400'}`}>
                                                    {item.name}
                                                </h3>
                                                <span className="font-bold text-cyan-400 bg-cyan-950/50 px-2 py-0.5 border border-cyan-800 text-sm shrink-0">
                                                    {item.price}
                                                </span>
                                            </div>
                                            <p className="text-gray-300 text-xs md:text-sm leading-snug line-clamp-2 [text-shadow:0_0_10px_rgba(0,0,0,0.8)]">
                                                {item.desc}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                                            ...
                                            {isSelected ? (
                                                <span className="text-yellow-400 text-xs font-bold animate-pulse flex items-center gap-1">
